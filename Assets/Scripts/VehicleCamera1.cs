...
public class VehicleCamera1 : MonoBehaviour
{
    ...
    public void PoliceLightSwitch()
    {
        ...
        if (!target.gameObject.GetComponent<PoliceLights1>()) return;
        ...
        if (PLValue == 1)
            target.gameObject.GetComponent<PoliceLights1>().activeLight = true;

        if (PLValue == 0)
            target.gameObject.GetComponent<PoliceLights1>().activeLight = false;
